...
// Retrieve post data based on post ID from URL
$post_id = isset($_GET['id']) ? $_GET['id'] : 0;
$query = "SELECT researcher_posts.*, researchers.full_name 
          FROM researcher_posts 
          INNER JOIN researchers ON researcher_posts.user_id = researchers.user_id 
          WHERE researcher_posts.id = $post_id";
...
$imgpath = !empty($post['imgpath']) ? $post['imgpath'] : 'img/default-image.png';
?>
        ...
        <div class="container-fluid header bg-white p-0">
            <div class="row g-0 align-items-center flex-column-reverse flex-md-row">
                <div class="col-md-6 p-5 mt-lg-5">
                    <h1 class="display-5 animated fadeIn mb-4"><?php echo htmlspecialchars($post['research_title']); ?>
                    </h1>
                    <ul>
                        <li>
                            <h5 class="animated fadeIn">Research Field:
                                <?php echo htmlspecialchars($post['research_field']); ?></h5>
                        </li>
                        <li>
                            <h5 class="animated fadeIn">Publications: <a
                                    href="<?php echo htmlspecialchars($post['publication_link']); ?>"><?php echo htmlspecialchars($post['publication_link']); ?></a>
                            </h5>
                        </li>
                    </ul>
                    <hr>
                    <h6>by</h6>
                    <h5><?php echo htmlspecialchars($post['full_name']); ?></h5>
                    <p>Date: <?php echo date('d/m/Y', strtotime($post['created_at'])); ?></p>
                </div>

                <div class="col-md-6 animated fadeIn">
                    <img class="img-fluid"
                        src="<?php echo !empty($post['imgpath']) ? $post['imgpath'] : 'img/property-3.png'; ?>" alt="">
                </div>
            </div>
        </div>
        <!-- Header End -->
        ...
        <div class="container-xxl py-5">
            <div class="container">
                ...
                <div class="tab-content">
                    ...
                    <h4>Methodology</h4>
                    <p><?php echo htmlspecialchars($post['research_methodology']); ?></p>
                    <hr>
                    <h4>Results</h4>
                    <p><?php echo htmlspecialchars($post['research_results']); ?></p>
                    <hr>
                    <h4>Conclusion</h4>
                    <p><?php echo htmlspecialchars($post['conclusion']); ?></p>
